...
in order get a sparse block of prepared variables from a sparse block of variables.
...
The VarKeys can be either integers, represented using a ValueSet structure, or alphanumeric values.
  represented using a ValueSetC structure.
...
either Numerical variables (`Block(Numerical)`) or Categorical variables (`Block(Catagorical)`).

The `VarKeys` used in both operands must be of the same type, eaither all integers or all alphanumerical values.
...
similar to the [`DataGridStats`](recoding-rules.md#cellindex) rule in the case of dense variables.

...
The entry point of Khiops Visualization Desktop is the khj file generated by Khiops at the end of the analysis. 
A double-click on this file opens Khiops Visualization Desktop.

![](../assets/images-khiops-guides/visualization/Opening.png)

Khiops Visualization Desktop is composed of several tabbed panes.

Depending on the analysis type, the panes and their contents are not the same. Khiops Visualization Desktop can be composed with 5 panels:
...
In the case of a variable with many categories, a specific bin ("Default group index") may gather many modalities and is shown with a different color. 

This specific bin is given the name of its most populated modality and clicking on the bin makes its content is available in panel E. As it may gather many small modalities, it may appear as the dominant bin of the distribution and the name might be misleading, therefore the different color.

The yLin/yLog toggle allows to display the y-axis either in linear scale or in log scale.

<!-- Complement sur Change all scales -->

![](../assets/images-khiops-guides/visualization/UnsupervisedFivePanels.png)
...
![](../assets/images-khiops-guides/visualization/UnsupervisedFivePanelsNumerical.png)
...
An additional button enables to choose the accuracy of numerical variables histograms.

By default, the histogram displayed is the best balance between accuracy and interpretability.

![](../assets/images-khiops-guides/visualization/DefaultCapitalLossHistogram.png)

For some datasets, a finest histogram is available but it can be difficult to interpret in case of many integer values.

![](../assets/images-khiops-guides/visualization/FinestCapitalLossHistogram.png)

Coarser histograms than default one are also available and can be useful to display smoother histograms.
![](../assets/images-khiops-guides/visualization/CoarsestCapitalLossHistogram.png)

The implemented algorithm allows to plot informative histograms of challenging heavy-tailed distributions such as Cauchy distribution. 

The figure below shows the default interpretable histogram of this distribution in log-log scale.

![](../assets/images-khiops-guides/visualization/CauchyLogLog.png)
...
This discretization is the cartesian product of the parts of each variable.
By default, the information displayed on the subtab "Matrix" is the mutual information between both groupings, allowing to analyze the correlations between the variables of the pair. 
Other information such as the frequencies in each cell or conditional probabilities can be selected from the menu.

The subtab "Cells" displays the information about the cell statistics as a table.

![](../assets/images-khiops-guides/visualization/Unsupervised-2D.png)

### Project information

This tab shows information related to the project, in particular, the content of the "Short description" field.

![](../assets/images-khiops-guides/visualization/ProjectInfos.png)
...
![](../assets/images-khiops-guides/visualization/ClassificationPreparation.png)
...
![](../assets/images-khiops-guides/visualization/SupervisedTree.png)
...
This discretization is the cartesian product of the parts for each variable. 

By default, the information displayed on the subtab "Matrix" is the frequency of each cell. 
Other information are the frequencies for a target value and the conditional probabilies.
...
![](../assets/images-khiops-guides/visualization/Supervised2D.png)

#### Modeling

The general layout is similar to the Preparation tab. Panel B shows information related to those variables used in the model only. 
The table shows for each variable its level, weight (in the weighted naive Bayes model) and its importance, computed as the mean of the absolute Shapley values.

![](../assets/images-khiops-guides/visualization/SupervisedModeling.png)
...
![](../assets/images-khiops-guides/visualization/SupervisedEvaluation.png)

#### Project information

This tab shows information related to the project, in particular, the content of the "Short description" field.

![](../assets/images-khiops-guides/visualization/SupervisedProject.png)
...
Clicking on a line of the table in panel B displays the information about the parts obtained from the optimal supervised grouping the selected variable selected in panel C and
information about the variable (Name and derivation rule (if any)) in panel D.
...
![](../assets/images-khiops-guides/visualization/RegressionPreparation.png)
...
![](../assets/images-khiops-guides/visualization/RegressionModeling.png)
...
Panel B gives some performance indicators for the model: rmse, mae, nlpd (
see [Negative log predictive density:octicons-link-external-16:][negative-log-predictive-density]{:target="_blank"} for details) 
and their equivalents for rank regression (for the rank negative log predictive density, 
see [this paper](https://www.jmlr.org/papers/volume8/hue07a/hue07a.pdf) for details).

[negative-log-predictive-density]: https://en.wikipedia.org/wiki/Negative_log_predictive_density "Visit the Wikipedia page"

Panel C shows the REC curves (for details about REC curves, see [this paper](https://dl.acm.org/doi/10.5555/3041838.3041844): the target modality can be chosen from the menu at the top and the "Filter curves" menu allows to choose which curves are displayed.

![](../assets/images-khiops-guides/visualization/RegressionEvaluation.png)
...
![](../assets/images-khiops-guides/visualization/TopRightButtons.png)
...
![](../assets/images-khiops-guides/visualization/TopRightPanelAButtons.png)
...
![](../assets/images-khiops-guides/visualization/Cooccurence.png)
